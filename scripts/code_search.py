...
def search_repo(
    ...
    """
    Search for repositories using semantic embeddings.

    QUERY: The search query to find relevant repositories (e.g., "detect oil rigs")
    """
    ...
    try:
        ...
        click.echo(f"\nSearch complete. Found {len(results)} results.")
        click.echo(repo_finder.print_results(results))
...
def validate_setup():
    ...
    try:
        from gcd.embeddings import Embedder  # noqa
    ...
    try:
        from gcd.search import MultiRepoCodeSearcher  # noqa
