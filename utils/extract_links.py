...
def clean_text_for_links(text: str) -> str:
    ...
    text = re.sub(r"https.*?\.com", lambda m: ''.join(m.group().split()), text, flags=re.DOTALL)
...
def extract_links_from_dataframe(df: pd.DataFrame, all_columns: bool = False) -> list[str]:
...
def extract_links_from_pdf_folder(pdf_folder: str, output_csv: str = "extracted_github_links.csv"):
    """
