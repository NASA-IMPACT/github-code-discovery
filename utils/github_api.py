...
def get_github_repo_info(repo_url: str, token: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not repo_resp.ok:
        raise Exception(f"Failed to fetch repo info from {repo_url}: {repo_resp.status_code} {repo_resp.text}")
    ...
    owner_name = None
    if owner_login:
        owner_info = requests.get(f"https://api.github.com/users/{owner_login}", headers=headers).json()
        owner_name = owner_info.get("name", owner_login)

    # Get contributors' actual names
    contributors_resp = requests.get(repo_info.get("contributors_url", ""), headers=headers)
...
def get_github_readme(repo_url: str, token: Optional[str] = None) -> Dict[str, str]:
    headers = {"Accept": "application/vnd.github.v3+json"}
    if token:
        headers["Authorization"] = f"token {token}"
    ...
    if not resp.ok:
        raise Exception(f"Failed to fetch README from {repo_url}: {resp.status_code} {resp.text}")
